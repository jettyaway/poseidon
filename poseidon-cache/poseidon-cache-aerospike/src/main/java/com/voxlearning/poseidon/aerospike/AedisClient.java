...
public class AedisClient {
    ...
    // in milliseconds
    private static final long AS_TIME_OFFSET = 1262304000000L;
    ...
    public AedisClient(String namespace, String set, String host, int port) {
        this();
    ...
    public AedisClient(String name, String namespace, String set, String host, int port, final int timeout) {
        this(namespace, set, host, port);
        setTimeout(timeout);
    }
    ...
    public List<String> hmget(String key, String... fields) {
        Key asKey = new Key(this.namespace, this.redisSet, key);
        List<Object> result = (List<Object>) this.asClient.execute(this.writePolicy, asKey, "redis", "HMGET", Value.get(this.redisBin),
    ...
    public boolean hexists(String key, String field) {
        Key asKey = new Key(this.namespace, this.redisSet, key);
        Long result = (Long) this.asClient.execute(this.writePolicy, asKey, "redis", "HEXISTS", Value.get(this.redisBin),
                Value.get(field));
        return (result == 1L);
    }
    ...
    public List<String> hvals(String key) {
        Key asKey = new Key(this.namespace, this.redisSet, key);
        List<Object> result = (List<Object>) this.asClient.execute(this.writePolicy, asKey, "redis", "HVALS", Value.get(this.redisBin));
        List<String> values = new ArrayList<>();
        for (Object obj : result) {
            values.add(String.valueOf(obj));
        }
    ...
    @SuppressWarnings("unchecked")
    public Map<String, Object> hgetAllObj(String key) {
        Key asKey = new Key(this.namespace, this.redisSet, key);
        List<Object> result = (List<Object>) this.asClient.execute(this.writePolicy, asKey, "redis", "HGETALL", Value.get(this.redisBin));
        if (result.size() % 2 != 0) {
            throw new AerospikeException("Redis hgetall: Keys and values mismatch");
        ...
        for (int i = 0; i < result.size(); i++) {
            if (i % 2 == 0) {
                keyString = (String) result.get(i);
            } else {
                mapResult.put(keyString, result.get(i));
            }
        }
